...
from backend.core import run_expert_analysis, run_summary_analysis, PROMPT_CONFIGS
from backend.db import init_db, get_db, set_db_config, get_db_config, test_db_connection, execute_query, get_db_type
...
def verify_password(plain_password: str, hashed_password: str) -> bool:
    ...
    print(f"[DEBUG]   Input password: {plain_password}")
    print(f"[DEBUG]   Hashed input: {hashed_input}")
    print(f"[DEBUG]   Stored hash: {hashed_password}")
    print(f"[DEBUG]   Match: {hashed_input == hashed_password}")
...
def get_user(username: str):
    ...
    print(f"[DEBUG] db_config: {get_db_config()}")
    with get_db() as conn:
        cursor = execute_query(
            conn, "SELECT * FROM users WHERE username = ?", (username,)
        )
        user = cursor.fetchone()
        print(f"[DEBUG] Found user (raw): {user}")
        # 转换为字典
        ...
        print(f"[DEBUG] Found user (dict): {user}")
    return user

def authenticate_user(username: str, password: str):
    print(f"[DEBUG] authenticate_user called with username: {username}, password: ***")
...
@app.post("/api/upload")
async def upload_file(file: UploadFile = File(...), current_user: dict = Depends(optional_auth)):
    try:
...
@app.post("/api/config")
def save_config_endpoint(config: dict = Body(...), current_user: dict = Depends(optional_auth)):
    try:
        save_config(config)
        return {"success": True}
    except Exception as e:
...
def get_history(limit: int = 20, current_user: dict = Depends(optional_auth)):
    try:
        with get_db() as conn:
            ...
            if current_user:
                cursor = execute_query(
                    conn, "SELECT id, created_at, title, input_content FROM reports WHERE user_id = ? ORDER BY created_at DESC LIMIT ?",
                    (current_user["id"], limit,),
                )
            else:
                cursor = execute_query(
                    conn, "SELECT id, created_at, title, input_content FROM reports ORDER BY created_at DESC LIMIT ?",
                    (limit,),
                )
    ...
    try:
        ...
        with get_db() as conn:
            ...
            if get_db_type() == "mysql":
                ...
                report_id = execute_query(conn, "SELECT last_insert_rowid()").fetchone()[0]
...
def update_history_title(report_id: int, data: dict = Body(...), current_user: dict = Depends(optional_auth)):
    try:
        with get_db() as conn:
            execute_query(conn,
...
async def read_users_me(current_user: dict = Depends(get_current_active_user)):
    return {"username": current_user["username"], "email": current_user.get("email"), "role": current_user.get("role", "user")}
...
async def get_users(current_user: dict = Depends(get_current_active_user)):
    try:
        ...
        with get_db() as conn:
            cursor = execute_query(conn, "SELECT id, username, email, created_at, last_login, role FROM users")
...
async def get_user_config(username: str, current_user: dict = Depends(get_current_active_user)):
    try:
        ...
        with get_db() as conn:
            cursor = execute_query(conn, "SELECT * FROM user_configs WHERE user_id = ?", (target_user["id"],))
            ...
            return {
                ...
                "model": config["model"],
                "prompt_configs": loads(config["prompt_configs"]) if config["prompt_configs"] else None
            }
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error getting user config: {str(e)}")

@app.post("/api/users/{username}/config")
async def save_user_config(username: str, config: UserConfigUpdate, current_user: dict = Depends(get_current_active_user)):
    try:
        ...
        with get_db() as conn:
            # 检查是否已存在配置
            cursor = execute_query(conn, "SELECT id FROM user_configs WHERE user_id = ?", (target_user["id"],))
            existing = cursor.fetchone()
            
            prompt_configs_json = dumps(config.prompt_configs) if config.prompt_configs else None
            ...
            if existing:
                execute_query(conn,
                    ...
                    WHERE user_id = ?
                    """,
                    (config.api_key, config.base_url, config.model, prompt_configs_json, now, target_user["id"])
                )
            else:
                execute_query(conn,
                    """
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                    """,
                    (target_user["id"], config.api_key, config.base_url, config.model, prompt_configs_json, now, now)
                )
            conn.commit()
...
async def update_user(user_id: int, user_data: UserUpdate, current_user: dict = Depends(get_current_active_user)):
    try:
        if current_user.get("role") != "admin":
            raise HTTPException(status_code=403, detail="Not enough permissions")
        ...
        with get_db() as conn:
            # 检查用户是否存在
            cursor = execute_query(conn, "SELECT id FROM users WHERE id = ?", (user_id,))
...
async def get_all_reports(current_user: dict = Depends(get_current_active_user)):
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error getting all reports: {str(e)}")
